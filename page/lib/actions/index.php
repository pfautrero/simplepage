		...
		if ($page) {
			...
			foreach($course_modules as $course_module) {
				...
				if ($PAGE->user_is_editing() && has_capability('moodle/course:manageactivities', $coursecontext)) {
					...
					if ($course_module['visible'] == 0) {
						$Column2[$course_module['position']][$course_module['sortorder']]['header'] .= "
							<img class='showhideactivities hideactivity' style='margin:2px;' src='".EYE_CLOSED."' alt='montrer-cacher' title='montrer-cacher' />";
					}
					else {
						$Column2[$course_module['position']][$course_module['sortorder']]['header'] .= "
							<img class='showhideactivities' style='margin:2px;' src='".EYE_OPENED."' alt='montrer-cacher' title='montrer-cacher' />";
					}
					$Column2[$course_module['position']][$course_module['sortorder']]['header'] .="
						<img class='deleteitem' style='margin:2px;' src='".DELETE."' alt='supprimer' title='suppression de cet item' />";
					
					$Column2[$course_module['position']][$course_module['sortorder']]['header'] .="
						<img class='duplicate' style='margin:2px;' src='".DUPLICATE."' alt='dupliquer' title='Afficher également sur une autre page' />";
				}			
				...
				// ==================== Module choice
					...
					if ($course_module['object']->name) {
						$Column2[$course_module['position']][$course_module['sortorder']]['content'] .= "
							<div class='boite-dial' style='margin:5px auto;'><div class='boite-head-sondage'><span>Sondage</span><p><a href='".$CFG->wwwroot."/mod/choice/view.php?id=".$course_module['cmid']."'>".$course_module['object']->name."</a></p></div></div>";
					}
				}
				// ==================== Module forum
					...
					if ($course_module['object']->name) {
						$Column2[$course_module['position']][$course_module['sortorder']]['content'] .= "
							<div class='boite-dial' style='margin:5px auto;'><div class='boite-head-forum'><span>Forum</span><br><a href='".$CFG->wwwroot."/mod/forum/view.php?id=".$course_module['cmid']."'>".$course_module['object']->name."</a></div></div>";
					}
				}
				// ==================== Module quiz
					...
					if ($course_module['object']->name) {
						$Column2[$course_module['position']][$course_module['sortorder']]['content'] .= "
							<div class='boite-dial' style='margin:5px auto;'><div class='boite-head-test'><span>Test</span><br><a href='".$CFG->wwwroot."/mod/quiz/view.php?id=".$course_module['cmid']."'>".$course_module['object']->name."</a></div></div>";
					}
				}
				// ==================== Module wiki
					...
					if ($course_module['object']->name) {
						$Column2[$course_module['position']][$course_module['sortorder']]['content'] .= "
							<div class='boite-dial' style='margin:5px auto;'><div class='boite-head-wiki'><span>Wiki</span><br><a href='".$CFG->wwwroot."/mod/wiki/view.php?id=".$course_module['cmid']."'>".$course_module['object']->name."</a></div></div>";
					}
				}
				// ==================== Module data
					...
					if ($course_module['object']->name) {
						$Column2[$course_module['position']][$course_module['sortorder']]['content'] .= "
							<div class='boite-dial' style='margin:5px auto;'><div class='boite-head-bdd'><span>Base de données</span><br><a href='".$CFG->wwwroot."/mod/data/view.php?id=".$course_module['cmid']."'>".$course_module['object']->name."</a></div></div>";
					}
				}
				// ==================== Module feedback
					...
					if ($course_module['object']->name) {
						$Column2[$course_module['position']][$course_module['sortorder']]['content'] .= "
							<div class='boite-dial' style='margin:5px auto;'><div class='boite-head-feedback'><span>Feedback</span><p><a href='".$CFG->wwwroot."/mod/feedback/view.php?id=".$course_module['cmid']."'>".$course_module['object']->name."</a></p></div></div>";
					}
				}
				// ==================== Module chat
					...
					if ($course_module['object']->name) {
						$Column2[$course_module['position']][$course_module['sortorder']]['content'] .= "
							<div class='boite-dial' style='margin:5px auto;'><div class='boite-head-chat'><span>Chat</span><p><a href='".$CFG->wwwroot."/mod/chat/view.php?id=".$course_module['cmid']."'>".$course_module['object']->name."</a></p></div></div>";
					}
				}
				// ==================== Module non reconnu
				else {
				...
				for($i=0;$i < count($selected_column);$i++) {
					if ($PAGE->user_is_editing() && has_capability('moodle/course:manageactivities', $coursecontext)) {
						...
						if ($selected_column[$i]['display_mode'] == 2) {
							$Column[$key] .= "<div class='module_mode_unavailable'><div class='header_module'>".$selected_column[$i]['header']."</div>".$selected_column[$i]['content']."</div>";							
						}
						else {
							$Column[$key] .= "<div class='module_mode_edit'><div class='header_module'>".$selected_column[$i]['header']."</div>".$selected_column[$i]['content']."</div>";							
						}						
			...
			for($i = 0;$i < count($array_pages);$i++) {
				if ($array_pages[$i]->id == $page) {
					if ((isset($array_pages[$i-1])) && ($array_pages[$i]->showbuttons & PREVIOUS_LINK)) $prev_page = $array_pages[$i-1]->id;
