...
class ajaxmodulepositionAction extends Action {
    ...
    public function launch(Request $request, Response $response)
    {
		...
		if ($position == "leftposition") $_SESSION[$entry]['position'] = $position;
		if ($position == "rightposition") $_SESSION[$entry]['position'] = $position;
		if ($position == "centerposition") $_SESSION[$entry]['position'] = $position;
		$content = $position;
		$response->addVar('content', $content);
		$this->render($LOCAL_PATH."/lib/template/ajaxSuccess.php");
		$this->printOut();
